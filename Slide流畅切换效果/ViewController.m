...
/*
 网易新闻实现步骤:
 1.搭建结构(导航控制器)
 * 自定义导航控制器根控制器ViewController
 * 搭建ViewController界面(上下滚动条)
 * 确定ViewController有多少个子控制器,添加子控制器
 2.设置上面滚动条标题
 * 遍历所有子控制器
 3.监听滚动条标题点击
 * 3.1 让标题选中,文字变为红色
...
-(void)createUI{
    ...
    titleScrollView.frame=CGRectMake(0, 64, self.view.bounds.size.width, 44);
    titleScrollView.backgroundColor=[UIColor whiteColor];
    [self.view addSubview:titleScrollView];
    _titleScrollView=titleScrollView;
    
    UIScrollView *contentScrollView = [[UIScrollView alloc]init];
    contentScrollView.frame=CGRectMake(0, 44+64, self.view.bounds.size.width, self.view.bounds.size.height-64-44);
...
- (void)setUpTitleLabel
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        // 设置高亮文字颜色
//        label.highlightedTextColor = [UIColor colorWithRed:73/255.0 green:199/255.0 blue:182/255.0 alpha:1.0];
...
- (void)selectLabel:(UILabel *)label
{
    ...
    // 颜色恢复
    _selLabel.textColor = [UIColor grayColor];
